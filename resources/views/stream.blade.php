...
<style>
  ...
  <div class="hidden w-full" id="main-content">
    <div class="mb-2 max-w-[300px]">
      <label for="branch" class="block mb-2 text-sm font-medium animate-pilihcabang text-white">Cari :</label>
      <select id="branch" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg animate-pilihcabang focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5">
      </select>
    </div>
  ...
  <script>
    ...
    const shouldFilterFromChart = (label) => {
      return label.includes('Prev') || label.includes('Diff_Pressure');
    };
    ...
    const getAllBranches = async () => {
      ...
      if (branches.length === 0) {
        ...
        document.getElementById('main-content').innerHTML = '<div class="text-center text-white p-4">Anda tidak memiliki akses ke cabang manapun. Silahkan hubungi admin.</div>';
        return [];
      }
    ...
    const loadData = async () => {
      ...
      const newSnapshot = newData.reduce((prev, curr) => {
        if (!shouldFilterFromChart(curr.pointName)) {
          return {
            ...prev,
            [curr.pointName]: [curr],
          };
      ...
      newData.forEach((d) => {
        if (!shouldFilterFromChart(d.pointName)) {
    ...
    window.onload = async function() {
      ...
      document.getElementById('branch').addEventListener('change', function(e) {
        ...
        const allLabels = labelsByBranch[selectedBranch];
        const chartLabels = labelsByBranch[selectedBranch].filter(label => !shouldFilterFromChart(label));
        
        historyBySource = allLabels.reduce((prev, curr) => {
          return {
            ...prev,
            [curr.split('.')[2]]: {
              ...prev[curr.split('.')[2]],
        ...
        document.getElementById('chart-data').innerHTML = sources.map(source => {
          let a = `<div class='px-[20px] rounded-xl py-[16px] bg-white mb-[12px] text-black'>
            <span class="rounded-sm font-bold text-[20px]">${source}</span>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 w-full mb-2 mt-2">`;
            ...
            if (labelShow[1] === source) {
              a = a + `
                <div class="block w-full p-3 bg-white border border-gray-200 rounded-lg shadow-sm">
                  ...
                  <canvas id="${label}-canvas" class="w-full h-[240px]"></canvas>
                </div>
              `;
            }
          }).join('');
